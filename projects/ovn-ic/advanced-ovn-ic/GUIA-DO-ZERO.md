...
O projeto roda em **três VMs**: `az1` e `az2` simulam dois datacenters/zonas diferentes e, dentro de cada uma, rodam **containers Incus** (os "workloads": frontend, backend, banco, monitoração e gerador de carga). A terceira, `quorum`, não é uma zona — ela existe só para dar o terceiro voto ao cluster de bancos do interconnect (explicado em 3.7).

### 2.2 Incus

**Incus** é um gerenciador de containers/VMs (parecido com Docker, mas mais próximo de "VM leve"). Aqui ele só é usado para criar os containers — a parte de rede (OVN) é conectada manualmente, não pelo Incus.
...
- **Plano mgmt** (`10.20.x.0/24`): a "rede de gerência de serviço", onde vivem o banco de dados e a monitoração.
...
- O daemon `ovn-ic` (um por AZ) sincroniza: copia os transit switches do IC-NB para o NB local de cada AZ, registra os gateways de cada AZ no IC-SB, e propaga as **rotas aprendidas** (ex: "AZ1 sabe que `10.10.2.0/24` fica atrás do gateway de AZ2").
- Cada roteador local de cada plano (`lr-client-az1`, `lr-mgmt-az1`, e os equivalentes em AZ2) ganha uma porta extra plugada no transit switch **do seu plano**, e é isso que permite tráfego de `10.10.1.0/24` (AZ1) chegar a `10.10.2.0/24` (AZ2). O plano mgmt faz o mesmo pelo `ts-mgmt`, separadamente.
...
- O **SNAT** permite que os containers privados saiam para a internet (troca o IP interno pelo IP externo antes de sair).
...
- **Managed nodes**: as máquinas configuradas — aqui, `az1`, `az2` e `quorum`. Só precisam de SSH e Python.
- **Inventory** (`inventory.ini`): lista de hosts e grupos (aqui: `azs` = `az1`+`az2`, e `ic_cluster` = as três).
...
- **Handler**: task que só roda se **notificada** por outra que mudou algo.
...
```
 ...
 PLANO CLIENT  10.10.1.0/24  ── ts-client ──  10.10.2.0/24
  ...
  • load-vm   gerador de carga        • VIP 10.10.2.100 (backend)
  • VIP 10.10.1.100 (frontend)
  • VIP 10.10.1.200 ──────────────────► backend em AZ2 (cross-AZ)

 PLANO MGMT    10.20.1.0/24  ── ts-mgmt  ──  10.20.2.0/24
  • app-vm-1 (2ª placa)               • app-vm-2 (2ª placa)
  • obs-vm  Prometheus + Grafana      • obs-vm  Prometheus + Grafana
  • load-vm (2ª placa)                • db-vm  Postgres :5432
...
- **Underlay** (camada física): as VMs se enxergam pelos IPs privados; o túnel GENEVE roda ali.
- **Overlay lógico** (OVN): dois switches e dois roteadores por AZ, mais um roteador de borda; cada plano ligado ao seu transit switch.
- **Aplicação**: containers Incus plugados direto no `br-int`, rodando React+nginx (AZ1), backend Java (AZ2) e PostgreSQL (AZ2, plano mgmt).

### 5.3 As rotas de tráfego que o projeto testa
...
2. **Consulta ao banco pelo plano mgmt**: `app-vm-1` (AZ1) → `db-vm` (AZ2) pelo `ts-mgmt` — aqui o MTU/GENEVE pesa.
3. **Comparação intra-AZ**: `app-vm-2` (AZ2) → `db-vm` (AZ2), caminho local. Medido: **2,9 ms cross-AZ vs 0,06 ms intra-AZ**.
4. **North-south**: cada plano sai para a internet pelo roteador de borda, com seu próprio endereço de SNAT.
5. **Isolamento (uma rota que NÃO pode funcionar)**: o plano client não alcança o banco, em nenhuma direção.

### 5.4 Como os bancos OVN se organizam
...
|---|---|
| `common` | Instala dependências, **compila o OVN do zero** (o pacote Ubuntu não inclui o `ovn-ic`) e instala a política de **logrotate** que impede um daemon em loop de encher o disco. |
| `ic_cluster` | Sobe IC-NB/IC-SB como **cluster RAFT de 3 membros** (az1, az2, quorum). |
| `ovn_central` | Cria os bancos NB/SB e sobe `ovn-northd` em cada AZ. |
| `ovn_chassis` | Configura o OVS local (encap-ip do GENEVE), sobe `ovn-controller`, cria o `br-ex`, habilita roteamento + MASQUERADE, e sobe o **agente de sonda** e o node_exporter do host. |
| `ovn_topology` | Cria a identidade da AZ, os **dois** transit switches no IC-NB, e por plano: LS/LR/LRP, gateway chassis, conexão ao transit switch. É onde moram as "pegadinhas". |
| `ovn_services` | Cria o **roteador de borda** (uplink, SNAT por plano, Edge Firewall) e os **load balancers**. |
| `incus` | Instala o Incus e roda `admin init --minimal`. |
| `workloads` | Cria as portas lógicas OVN (**uma por placa**), os containers com uma interface `p2p` por plano no `br-int`, configura IP/MTU/rotas via netplan, e sobe as camadas: PostgreSQL → backend Java → publicação do estado da infra → Prometheus/Grafana → frontend React → gerador de carga. |
...
chassis.yml    -> OVS chassis + ovn-controller + br-ex + sonda + node_exporter
topology.yml   -> os dois planos + os dois transit switches + ovn-ic
services.yml   -> roteador de borda + load balancers
incus.yml      -> instala e inicializa Incus
workloads.yml  -> containers (2 placas) + Postgres + Java + React + Prometheus/Grafana + carga
...
1. A identidade de uma AZ vem de `NB_Global.name`.
2. Os transit switches só podem ser criados no IC-NB, nunca no NB local.
3. É preciso reiniciar `ovn-ic` depois que a topologia estiver pronta (condição de corrida).
4. As opções de rota (`ic-route-adv`/`ic-route-learn`) ficam em `NB_Global.options`, não no roteador.
5. É preciso fixar um "gateway chassis" na porta de trânsito, senão as rotas são anunciadas mas o plano de dados não funciona.
6. O `ovn-ic` compilado precisa de `--unixctl` explícito, senão entra em crash-loop.
7. O cliente OVS do Incus 6.0 não conecta no `br-int`; por isso as placas são `p2p` conectadas à mão.
8. As tabelas são criadas pelo superusuário `postgres`, então é preciso `GRANT` explícito para `appuser` — inclusive **`DELETE`**, usado ao republicar o estado da infra.
9. O `ovn-controller` precisa que `/run/ovn` exista e — diferente do `ovn-ic` — **não tem** opção `--unixctl`. Como `/run` é tmpfs, depois de um reboot o diretório some e ele entra em crash-loop. A correção é `RuntimeDirectory=ovn`. Esse loop chegou a escrever **8 GB de log e encher o disco de 38 GB**, derrubando o laboratório inteiro.
10. O OVN **não programa load balancer** em roteador com mais de uma porta de gateway distribuída. Por isso o north-south foi para um roteador de borda separado.
11. Um VIP dentro da sub-rede do tenant precisa do LB **no switch** — só assim alguém responde ao ARP do VIP.
12. …mas o LB no switch é **ignorado** para tráfego vindo de um roteador. Então clientes externos precisam do LB também num roteador.
13. `lb_force_snat_ip=router_ip` é rejeitado nesta versão; tem que ser o IP literal.
14. Um VIP **só funciona de dentro da AZ que o hospeda**. Por isso AZ1 tem um VIP de serviço (`10.10.1.200`) apontando para o backend em AZ2.
15. Um VIP na sub-rede do tenant precisa do LB **no roteador além do switch**, senão ninguém responde ao ARP e o cliente nem envia o primeiro pacote.
...
ansible-playbook site.yml      # ou estágio por estágio, ver 5.6
...
Pelo painel: abra `ssh -L 8080:10.10.1.100:80 az1` → `http://localhost:8080/` e use os botões **"Testar"** — cada clique roda as verificações de verdade naquele momento.
...
- **OVN controlado manualmente, Incus só cria os workloads**: o Incus tem rede OVN nativa, mas gerencia o NB sozinho e não expõe configuração de OVN-IC.
- **Containers, não VMs**: os hosts são pequenos; containers sobem rápido e passam pelo mesmo caminho GENEVE/MTU.
- **Load balancer nativo do OVN em vez de HAProxy**: menos peças móveis, demonstra um recurso do próprio OVN.
- **Um único Postgres em AZ2 (sem réplica)**: simplicidade proposital — gera de graça o contraste de latência intra vs. inter-AZ.
- **Aplicação dividida entre as AZs de propósito**: assim o fluxo testado é o tráfego real do produto, não uma sonda artificial. Se o interconnect quebrar, o dashboard avisa em segundos.
...
Persistir configuração de rede do host após reboot, `ansible-vault` para a senha do banco, health checks no load balancer, Postgres primary/replica, TLS nos bancos, ACLs de verdade nos switches, federação Prometheus cross-AZ, `blackbox_exporter` com sondas que devem falhar, injeção de falha (`tc netem` no túnel), suporte a mais de 2 AZs, workloads como VMs Incus, e `molecule`/`ansible-lint` no CI.

(A alta disponibilidade dos bancos IC **já foi feita** — é o cluster RAFT da seção 3.7.)

---

## Parte 6 — Observabilidade e geração de carga

### 6.1 Onde a monitoração mora, e por quê

Cada AZ tem seu **próprio** Prometheus + Grafana, dentro do container `obs-vm`, **no plano de gerência**. Duas razões:

- é exatamente para isso que existe uma "rede de gerência de serviço" — o tráfego de monitoração não se mistura com o do cliente;
- cada célula coleta **só os próprios alvos**, então uma falha numa AZ não cega a monitoração da outra, e a coleta não pesa no interconnect.

Detalhe que vale entender: o backend Java serve a API no plano **client** e as métricas no plano **mgmt** — mesmo processo, dois listeners, dois planos. Não dá para raspar métricas a partir da rede do cliente, não por regra de firewall, mas porque não existe caminho.

### 6.2 O que é coletado

| Exporter | Onde | O que tem de interessante |
|---|---|---|
| node_exporter | hosts das AZs | bytes/s em `genev_sys_6081` — o tráfego do interconnect, literalmente |
| node_exporter | cada container | tráfego **por placa de rede**: plano client e mgmt lado a lado |
| agente de sonda | hosts das AZs | estado do OVN virando métrica (transit switches, RAFT, rotas) |
| backend Java | `app-vm-2` | req/s, histograma de latência, latência do banco |
| postgres_exporter | `db-vm` | commits, linhas inseridas, conexões |
| gerador de carga | `load-vm` | carga oferecida e latência vista pelo cliente |

### 6.3 De onde vem o tráfego

Sem carga, o dashboard não mostra nada. O container `load-vm` (em AZ1, com duas placas como as camadas da aplicação) fica batendo no **VIP do frontend** — ou seja, cada requisição passa por nginx → VIP de serviço → `ts-client` (GENEVE) → backend Java → PostgreSQL.

- **Mix de endpoints**: `/api/status`, `/api/report` (agregação pesada), `/api/orders` (INSERT de verdade) e `/api/health`. Assim o banco vê leitura e escrita, não uma consulta trivial.
- **Taxa em senoide** (um ciclo a cada 10 min), para o gráfico ter forma. Linha reta não conta nada sobre como o sistema reage a carga.
- **Carga no plano mgmt**: um probe periódico direto ao banco na outra AZ, pelo `ts-mgmt`. O caminho da aplicação nunca cruza o `ts-mgmt` (backend e banco estão os dois em AZ2), então sem isso esse transit switch não teria tráfego nenhum para medir.

### 6.4 Abrindo os painéis
...
São três dashboards: **Interconnect** (túnel GENEVE, RAFT, rotas), **Two planes** (tráfego client vs mgmt por workload — o banco nunca aparece no painel do plano client, porque não tem placa lá) e **Application** (req/s, percentis de latência, latência cross-AZ medida no cliente).

O Grafana é alcançado por um **proxy device do Incus**, não por uma rota do host para o plano de gerência — o encaminhamento acontece dentro do namespace do container, então o host nunca precisa de rota para `10.20.x` e o plano continua fechado.

A retenção do Prometheus é limitada em **tempo e tamanho** (`6h` e `1GB`). Limitar só por tempo não limita bytes — e disco cheio já derrubou este laboratório uma vez (gotcha #9).
...
| Termo | Significado |
|---|---|
| AZ | Availability Zone — aqui, cada VM (`az1`/`az2`) simula uma zona de disponibilidade |
| VPC | Rede virtual privada de um tenant, federada entre as AZs |
| LS / LR / LRP | Logical Switch / Router / Router Port — peças da topologia OVN |
| NB / SB | Northbound / Southbound DB — intenção declarada vs. estado operacional |
| IC-NB / IC-SB | Northbound/Southbound **globais** usados só para federação (OVN-IC) |
| Transit switch | Sub-rede especial que liga os roteadores de AZs diferentes |
